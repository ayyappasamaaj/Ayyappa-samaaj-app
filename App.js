...
import { View, Animated, StyleSheet, AppRegistry, Text, WebView, ActivityIndicator, NativeModules, TouchableHighlight, ScrollView } from 'react-native';
...
export default class App extends PureComponent<void, *, State> {
  state: State = {
    ...
    routes: [
      { key: '1', icon: 'ios-book' },
      { key: '2',  icon: 'ios-calendar' },
      { key: '3',  icon: 'logo-usd' },
      { key: '4',  icon: 'ios-information-circle' },
    ],
    eventsResponse: '',
    homeResponse: '',
    routeIndex : '',
...
_renderIcon = ({ route }) => {
    return <Ionicon name={route.icon} style={styles.icon} size={responsiveWidth(6)} reverse='true' />;
  };
